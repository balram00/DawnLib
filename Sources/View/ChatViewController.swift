...
class ChatViewControllers: UIViewController {
...
//    var chatItems: [ChatItem] = []
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == chatItems.count + 1 {
            // Footer Cell
            guard let footer = tableView.dequeueReusableCell(withIdentifier: ChatFooterCell.identifier) as? ChatFooterCell else {
                return UITableViewCell()
            }
            return footer
        }
        ...
        case .bulletPoints(let points, let underlineWords):
            guard let cell = tableView.dequeueReusableCell(withIdentifier: ResponseTableViewCell.identifier) as? ResponseTableViewCell else {
                return UITableViewCell()
            }
